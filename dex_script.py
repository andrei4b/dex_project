def solutions(words, each_min_len, full_len):
    ...
    #write to file
    outfile2 = open('solutii_' + str(each_min_len) + '-' + str(full_len) + '.txt', 'w')
    for s in solution:
        outfile2.write(s[0] + ' + ' + s[1] + '\n')

    outfile2.close()
    print("Generated " + 'solutii_' + str(each_min_len) + '-' + str(full_len) + '.txt')
    return solution
...
def valid(word,words):
    ...
    'poli', 'or', 'mu', 'fi', 'ga', 'pe', 'cur', 'ha', 'sur', 'col', 
    'ad', 'no', 'tele', 'bă', 'fa', 'ter', 'aero', 'să', 'pu', 'na', 'va', 
    'inter', 'câr', 'ti', 'dec', 'for', 'semi', 'sta', 'bur', 'șu', 'uni', 
    ...
    'ti', 'la', 'atic', 'ca', 'uș', 'car', 'ion', 'bă', 'eră', 'și', 
